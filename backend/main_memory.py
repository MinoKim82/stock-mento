...
# 메모리 캐시: 세션별 CSV 데이터와 TransactionParser 인스턴스
csv_cache: Dict[str, str] = {}  # session_id -> CSV content
parsers: Dict[str, TransactionParser] = {}  # session_id -> TransactionParser
...
# CSV 파일 업로드 및 메모리 캐싱 API
@app.post("/upload-csv", response_model=SessionInfoResponse)
async def upload_csv(file: UploadFile = File(...)):
    """CSV 파일을 메모리에 캐싱하고 세션 ID를 반환"""
    ...
    try:
        ...
        # CSV 내용을 메모리 캐시에 저장
        csv_cache[session_id] = csv_text
        
        # 임시 파일을 메모리에서 생성 (TransactionParser가 파일 경로를 요구하므로)
        temp_file_path = f"/tmp/{session_id}.csv"
        with open(temp_file_path, 'w', encoding='utf-8') as f:
            f.write(csv_text)
        
        # TransactionParser 인스턴스 생성
        parser = TransactionParser(temp_file_path, enable_real_time_prices=True, use_yahoo=True)
        parsers[session_id] = parser
        
        return SessionInfoResponse(
            session_id=session_id,
            message="CSV 파일이 성공적으로 메모리에 캐싱되었습니다.",
            cache_size=len(csv_text),
            total_sessions=len(csv_cache)
        )
    except Exception as e:
        # 오류 발생 시 캐시 정리
        if session_id in csv_cache:
...
def get_parser(session_id: str) -> TransactionParser:
    ...
    if parsers[session_id] is None:
        ...
        temp_file_path = f"/tmp/{session_id}.csv"
        with open(temp_file_path, 'w', encoding='utf-8') as f:
            f.write(csv_cache[session_id])
        
        # 파서 재생성
        parsers[session_id] = TransactionParser(temp_file_path, enable_real_time_prices=True, use_yahoo=True)
    ...
    try:
        ...
        for account in accounts:
            ...
            accounts_by_owner_and_type[owner][account_type].append({
                ...
                "balance": balance_info.cash_balance if balance_info else 0,
                "stock_value": balance_info.stock_value if balance_info else 0,
                "total_balance": balance_info.total_balance if balance_info else 0,
